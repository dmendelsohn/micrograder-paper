...
In the introduction of this paper, we discussed that the grader and the system being graded exist in separate computing environments.  To help us describe the implementation of MicroGrader, additional terminology is helpful.  We will refer to the grader, running on a full-fledged computer, as the MicroGrader \textit{server}.  We will refer to the embedded system being graded as the MicroGrader \textit{client}.  Let us first discuss these two components in broad terms, before continuing in greater detail.

The server uses a test's input frames to generates test inputs, communicates those inputs to the client when necessary, and collects observations from the client.  When the test's termination condition is met, the server uses the test's evaluation points to grade the observed embedded system outputs. 
...
\caption{Block diagram of the server assessing a program on the client.}
\label{fig:test-mode}
\end{figure}
