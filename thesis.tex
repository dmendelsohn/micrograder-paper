...
TODO: talk about what we have (input samples)...need to recover a continuous signal.  Discuss our various methods of doing that

TODO: now that we have a the continuous signal, we need to figure out the bounds of each frame.  Some frames might be discarded.  Describe.

TODO: Portion of signal within bounds is zero-ed, zero-order holded out to infinity.  Use a pair of plots here to demonstrate.  MicroGrader can then use its representation of this signal to construct the function.  Remind that this process is occurring for each frame and for each input channel. 
...
%\begin{itemize}
%\item Forward hold: a recorded value holds in the interval $[t_n, t_{n+1})$
%\item Backward hold: a recorded value holds in the interval $(t_{n-1}, t_n]$
%\item Bi-direction hold: a recorded value holds in the interval $[(t_{n-1} + t_n)/2, (t_n + t_{n+1})/2)$
%\item Interpolation: perform linear interpolation with a specified resolution between each sample.  Note that this doesn't work for certain input types (namely digital inputs).
%\end{itemize}

\subsection{Constructing evaluation points}
\label{sec:using-point-templates}
TODO: discrete to continuous

TODO: groundwork... for each frame and channel, we have a bounded signal on that channel to evaluate

For each (frame, channel) pair, we create a set of evaluation points.  Specifically, we create a new evaluation point each time the output value changes on that channel.  TODO: an image here would be helpful

TODO: details on the new point.  Mention how templates come into play.  Ignore the time interval thing I did in the template.  Include a table corresponding to above image

\subsection{Auto-generated test case for Wikipedia Scroller}
TODO: give some input plots from a real test case, and create table of evaluation points
