...
Each student implements a tilt-based interface that allows the device's user to build an alphanumeric query string.  Using the ESP8266 chip, the device transmits the query string to a server-side Python program.  That Python program uses the query to formulate a query to the Wikipedia API, parses the response from the Wikipedia API, and transmits the first 200 characters of the relevant Wikipedia description to the embedded device.
...
%\includegraphics[scale=0.125]{pigeons.png}
\vspace{5mm}
\caption{TODO: Figure and caption of simple digital signal}
\label{fig:static-input}
\end{figure}

In this particular example, presuming the digital input is connected to an active-low push-button switch, the test case indicates the button is pressed in the interval $(0,0)$ and $(0,0)$ (TODO: fill in values) and not pressed at all other times.  We would define $t=0$ as the time at which the microcontroller last powered on or reset.  In this static paradigm, the input signals would not depend upon the behavior of the embedded system.

In practice, we found that statically defined inputs limited scope of assignments for which MicroGrader could be used.  Grading assignments often requires input signals that are dependent on the behavior of the system.  Consider ``Wikipedia Scroller" for a concrete example.  In particular, Wikipedia Scroller requires that the system wait for an HTTP response after transmitting a query.  The length of time spent waiting for the response varies widely and is not within the student's control.  When human graders evaluate the system, they know to wait for the HTTP response, whether it takes one second or five seconds, before performing additional test actions.  We should consider the timing of subsequent actions relative to the arrival of the HTTP response, not relative to the initialization of the system as a whole.  We can conclude that, in order to effectively grade Wikipedia Scroller, MicroGrader test cases need to dynamically define inputs, with respect to run-time events.

\subsection{Dynamically defined inputs}
TODO: introduce concept of "primitive event".  But we need to describe unique events to make relative time make sense...

\subsubsection{Data structure: ``Condition"}
In order to specify unique run-time events, we must first introduce a data structure that describes those events.  We will refer to this data structure as a \textit{condition}.  As an embedded system executes a program, MicroGrader can determine the condition's $t_{\text{satisfied}}$, the elapsed time between the embedded system's initialization and the condition's satisfaction.  A condition's $t_{\text{satisfied}}$ can be undefined, signifying that the condition was never satisfied during the embedded system's execution.

Our condition data structure can express unique events such as: ``The first HTTP request", ``The second HTTP request", and ``The first button press after the second HTTP response".  It achieves this with a recursive design; complex conditions can be built by composing simpler ones.  There are four fundamental types of conditions.

The first type of condition is the \textit{basic condition}.  A basic condition's $t_{\text{satisfied}}$ is defined by a \textit{cause}. A cause is boolean function that takes a single input representing a MicroGrader primitive event.  As MicroGrader processes each primitive event during the execution of an embedded program, the cause function is invoked with that primitive event as its input.  The condition's $t_{\text{satisfied}}$ is the \textbf{first} time at which the cause function returns \texttt{true}.  TODO: give ``first HTTP request" example

The second type of condition is the \textit{after condition}.  After conditions have another condition (of any kind) as a ``precondition".  Like basic conditions, after conditions have a cause function.  This cause function is only invoked for primitive events that occur after the precondition is satisfied.  As before, the after condition's $t_{\text{satisfied}}$ is the first time at which the cause function returns \texttt{true}.  TODO: give ``second HTTP request" example
...
TODO: Give a big example with a sequence of primitive events and various conditions

\subsubsection{Data structure: ``Input Frame"}
TODO: introduce InputFrame, together with examples of the two inputs frames for Wikipedia Scroller.  Use images!  Fundamentally, input frame is function: (channel, t) -> value.  Channel is a label that specifies a signal (give relevant examples).  Values can have different forms depending on type (e.g. digital vs analog)
