...
\subsubsection{Stage 1: the interactive stage}
In the interactive stage, the server loads the relevant test case and waits for the client to connect.  Once the client connects, it begins executing the embedded program and sending messages to the server.  There are, broadly speaking, two types of message from the client to the server: observations and input requests.

TODO: describe observation messages better (timestamp + observation(?)).  When the server receives an observation message, the observation and its timestamp are logged.  In addition, for each of the test's input frames, the server examines the frame's start and end conditions, and updates them.  That is, it determines if the new observation causes these conditions to be met.  In this way, the server can keep track of which input frames are currently active, and which frame should be used to generate test inputs.  The test's termination condition is also examined, to determine if the interactive session should end.

An input request is a message from the client to the server that specifies one or more input channels for which a value is required.  Typically, this means that the student's program is attempting to sample a TODO: describe input requests.  When the server receiv...


\subsubsection{Stage 2: the evaluation stage}
...
In broad terms, the server uses a test's input frames to generates test inputs, communicates those inputs to the client when necessary, and collects observations from the client.  When the test's termination condition is met, the server uses the test's evaluation points to grade the observed embedded system outputs. 

TODO: describe why Python 3 was used for the server

\subsubsection{Role of the client}
TODO discuss overall role

TODO discuss how its a thin wrapper, and why


TODO describe how we have developed one implementation, but others are possible.

TODO: quickly describe our implementation and why we used Teensy

TODO: discuss general principles for a client.  Invisible communication layer + minimal changes to code.  Good idea to have an inactive mode
...
\subsection{Platform agnosticism (remove)}
