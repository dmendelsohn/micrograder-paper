...
TODO: paragraph to motivate this work 

Consider the Wikipedia Scroller exercise described in section \ref{sec:wiki-scroller}.  TODO: reminder of human process

TODO: describe manual test case construction process in two paragraphs, one for inputs one for outputs, in reference to "human process"
...
It is difficult to define a test case using observations alone.  There are many ambiguities in terms of how the instructor might want to the assignment to be graded. [TODO: give examples].  MicroGrader uses a data structure called the \textit{scaffold} to allow for instructors to specify these preferences.  The scaffold and the observations, when combined, can be used to produce a full test case.

\subsection{Frame templates}
TODO: describe

In our Wikipedia Scroller exercise, we can use a set of two frame templates for the evaluation procedure previously outlined.  The first frame could start when system initialization is complete, and end when the first HTTP request occurs.  The second frame could start when the first HTTP response arrives, and end when the recording ends.  This two-frame structure allows the test case to ignore the variable latency period of time when the embedded system is waiting for an HTTP response.  After all, the length of this time period should not be relevant in determining the correctness of a student's solution.  If we tried to use one frame for this whole evaluation, the student solution's timing after the variable-latency event would probably not match the staff solution's timing.

TODO: describe how frame templates get ``accepted" or ``rejected"

\subsection{Constructing test inputs}
