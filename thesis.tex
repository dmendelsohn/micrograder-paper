...
TODO: describe data flow with block diagram for "inactive" mode.

TODO: describe timestamp definition (embedded system clock is master)
...
In the interactive stage, the server loads the relevant test case and waits for the client to connect.  Once the client connects, the server processes each message from the client.  TODO: sentence about two types of messages

If the message is an observation, such as an embedded system output or an important internal event, the observation and its timestamp are logged.  TODO: in addition, each input frame's conditions are updated live
...
Removed, merged into other sections
